...
import {
  ...
  Target, Sparkles, BarChart3, Briefcase,
} from "lucide-react";
...
export default function PricingPage() {
  ...
  return (
    <div className="bg-[#0A0E1A] min-h-screen">
        ...
        <div className="w-full max-w-[1280px] mx-auto px-4 sm:px-6 relative z-10 text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div className="flex flex-wrap items-center justify-center gap-6 mt-8 text-sm text-gray-400">
              <span className="flex items-center gap-2"><Award size={16} className="text-[#2ECC71]" /> Trusted Globally</span>
              <span className="flex items-center gap-2"><Users size={16} className="text-[#2ECC71]" /> 50+ Clients Worldwide</span>
              <span className="flex items-center gap-2"><Clock size={16} className="text-[#2ECC71]" /> On-Time Delivery</span>
            </div>
          </motion.div>
        </div>
      </section>
      ...
      {/* ── Trust Bar ── */}
      <div style={{ padding: "1.5rem 1rem", background: "#0A0E1A", borderBottom: "1px solid #1A2540" }}>
        <div className="w-full max-w-[1280px] mx-auto px-4 sm:px-6">
          <div className="flex flex-wrap items-center justify-center gap-8 text-xs text-gray-500 uppercase tracking-wider">
            ...
            <span className="flex items-center gap-2"><span className="w-6 h-4 rounded-sm bg-red-500/20 border border-red-500/30 inline-block"></span> USA</span>
            <span className="flex items-center gap-2"><span className="w-6 h-4 rounded-sm bg-blue-500/20 border border-blue-500/30 inline-block"></span> UK</span>
            <span className="flex items-center gap-2"><span className="w-6 h-4 rounded-sm bg-yellow-500/20 border border-yellow-500/30 inline-block"></span> Germany</span>
            <span className="flex items-center gap-2"><span className="w-6 h-4 rounded-sm bg-green-500/20 border border-green-500/30 inline-block"></span> UAE</span>
            <span className="flex items-center gap-2"><span className="w-6 h-4 rounded-sm bg-orange-500/20 border border-orange-500/30 inline-block"></span> Kenya</span>
            <span className="flex items-center gap-2"><span className="w-6 h-4 rounded-sm bg-purple-500/20 border border-purple-500/30 inline-block"></span> Singapore</span>
            ...
            <div className={`grid gap-6 mb-10 ${category.tiers.length === 3 ? "md:grid-cols-3" : "md:grid-cols-2 max-w-3xl mx-auto"}`}>
              {category.tiers.map((tier, i) => (
                <motion.div key={tier.name} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  {tier.badge && (
                    <div className="absolute -top-3 left-4 text-[10px] font-semibold px-3 py-0.5 rounded-full"
                  ...
                  <div className="mb-4">
                    ...
                    <div className="text-white font-bold text-2xl leading-tight">
                      {tier.price}
                      {tier.priceTo && <span className="text-gray-400 text-lg font-normal"> – {tier.priceTo}</span>}
                    </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ borderBottom: "1px solid #1A2540" }}>
                  ...
                  <th className="text-left py-3 px-4 text-gray-400 font-semibold hidden sm:table-cell">Ideal For</th>
                  <th className="text-left py-3 px-4 text-gray-400 font-semibold hidden md:table-cell">Timeline</th>
                  <th className="py-3 px-4"></th>
                </tr>
              </thead>
              <tbody>
                {[
                  { label: "Discovery & Strategy Sprint", ideal: "Clarity & Fixed Price", time: "4 weeks", icon: Rocket, color: "#2ECC71", id: "discovery" },
                  { label: "MVP & Startup Launch", ideal: "Fast Market Entry", time: "2–3 months", icon: Zap, color: "#1A6BB5", id: "mvp" },
                  { label: "Full-Scale Platform Development", ideal: "Enterprise Scale", time: "4–8 months", icon: Layers, color: "#9B59B6", id: "platform" },
                  { label: "AI & Automation Solutions", ideal: "Intelligent Systems", time: "3–5 months", icon: Brain, color: "#00BCD4", id: "ai" },
                  { label: "Legacy Modernization & Security", ideal: "Migration & Compliance", time: "3–6 months", icon: Shield, color: "#E74C3C", id: "modernization" },
                  { label: "Dedicated Team Retainer", ideal: "Long-Term Partnership", time: "Ongoing", icon: Briefcase, color: "#F39C12", id: "retainer" },
                ].map((row) => (
      ...
      <section style={{ padding: "4rem 1.5rem", background: "#0A0E1A", borderTop: "1px solid #1A2540" }}>
        <div className="w-full max-w-[1280px] mx-auto px-4 sm:px-6">
          <div className="text-center mb-12">
          ...
          <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            <div className="rounded-2xl p-6 text-center" style={{ background: "#0F1629", border: "1px solid #1A2540" }}>
              <div className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-4" style={{ background: "rgba(46,204,113,0.1)" }}>
                <Target size={28} style={{ color: "#2ECC71" }} />
              </div>
              <h3 className="text-white font-bold text-lg mb-2">Discovery-First Approach</h3>
              <p className="text-gray-400 text-sm leading-relaxed">We start with a paid Discovery Sprint to give you clarity, a fixed-price quote, and confidence. No surprises, no hidden costs.</p>
            </div>
            <div className="rounded-2xl p-6 text-center" style={{ background: "#0F1629", border: "1px solid #1A2540" }}>
              <div className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-4" style={{ background: "rgba(26,107,181,0.1)" }}>
                <Users size={28} style={{ color: "#1A6BB5" }} />
              </div>
              <h3 className="text-white font-bold text-lg mb-2">Global Team, Local Attention</h3>
              <p className="text-gray-400 text-sm leading-relaxed">Our distributed team works seamlessly across time zones. You get world-class engineering with the personal attention of a dedicated partner.</p>
            </div>
            <div className="rounded-2xl p-6 text-center" style={{ background: "#0F1629", border: "1px solid #1A2540" }}>
              <div className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-4" style={{ background: "rgba(155,89,182,0.1)" }}>
                <BarChart3 size={28} style={{ color: "#9B59B6" }} />
              </div>
              <h3 className="text-white font-bold text-lg mb-2">Built to Scale</h3>
              <p className="text-gray-400 text-sm leading-relaxed">We engineer solutions that grow with you. From MVP to enterprise platform, your architecture is designed for scalability, security, and performance.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: "5rem 1.5rem", background: "#0F1629", borderTop: "1px solid #1A2540" }}>
        <div className="w-full max-w-[1280px] mx-auto px-4 sm:px-6">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            ...
            <h2 className="text-3xl sm:text-4xl font-black text-white mb-4">Let's Build Something Great Together</h2>
            <p className="text-gray-400 text-lg mb-8 max-w-xl mx-auto">
              Every great partnership starts with a conversation. Tell us about your project,
              and we'll recommend the best path forward — completely free, with no obligation.
            </p>
            ...
            <div className="flex flex-wrap items-center justify-center gap-6 mt-6 text-gray-500 text-sm">
              <span className="flex items-center gap-2"><Clock size={14} /> Response within 24 hours</span>
              <span className="flex items-center gap-2"><MessageCircle size={14} /> Free technical consultation</span>
              <span className="flex items-center gap-2"><TrendingUp size={14} /> No commitment required</span>
            </div>
          </motion.div>
        </div>
      </section>
